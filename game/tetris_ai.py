...
def draw_grid(background):
    ...
    for i in range(rows + 1):
        pygame.draw.line(background, (50, 50, 50), (0, i * block), (width, i * block), 1)
    # Draw vertical lines
    for j in range(columns + 1):
        pygame.draw.line(background, (50, 50, 50), (j * block, 0), (j * block, height), 1)
...
def ai_move(tetraminoe):
    """Ask the agent for the best rotation and column, then snap the piece there."""
    rotated_piece, col = agent.get_best_col_and_rotation(board, tetraminoe.shape)

    if rotated_piece is not None and col is not None:
        tetraminoe.shape = rotated_piece
        tetraminoe.column = col
...
def draw_side_panel(screen, next_tetraminoe, pieces_placed):
    """Draws the side panel with next piece preview and AI stats."""
    panel_x = width + 10
    font_title = pygame.font.SysFont('Arial', 16, bold=True)
    font_text  = pygame.font.SysFont('Arial', 14)

    # ── Panel background ──────────────────────────────────────────
    pygame.draw.rect(screen, (20, 20, 20), pygame.Rect(width, 0, panel_width, height))
    pygame.draw.line(screen, (80, 80, 80), (width, 0), (width, height), 2)

    # ── NEXT PIECE label ─────────────────────────────────────────
    label = font_title.render("NEXT", True, (200, 200, 200))
    screen.blit(label, (panel_x, 15))

    # ── Draw next piece preview ──────────────────────────────────
    preview_block = 20
    preview_top   = 40
    for i, row in enumerate(next_tetraminoe.shape):
        for j, cell in enumerate(row):
            if cell == 1:
                pygame.draw.rect(
                    screen,
                    next_tetraminoe.color,
                    pygame.Rect(
                        panel_x + j * preview_block,
                        preview_top + i * preview_block,
                        preview_block - 1,
                        preview_block - 1
                    )
                )
    ...
    # ── Divider ───────────────────────────────────────────────────
    pygame.draw.line(screen, (80, 80, 80), (width, 130), (width + panel_width, 130), 1)

    # ── AI stats ──────────────────────────────────────────────────
    stats = [
        ("AI MODE",   (0, 255, 0)),
        ("",          (255, 255, 255)),
        (f"Lines  : {score}",   (255, 255, 255)),
        (f"Pieces : {pieces_placed}", (255, 255, 255)),
    ]
    for i, (text, color) in enumerate(stats):
        surface = font_text.render(text, True, color)
        screen.blit(surface, (panel_x, 145 + i * 22))
	...
	while(is_using_program):
		...
		if fall_time/1000 >= fall_speed:
			...
			if falling(tetraminoe):
				...
				lines_before = score
				clear_lines(locked)
				lines_just_cleared = score - lines_before
				ai_move(tetraminoe)
				pieces_placed += 1
				logger.log_step(lines_cleared=lines_just_cleared, score=score)
		...
		for event in pygame.event.get():
			...
			# AI plays automatically — no keyboard controls
				pass

		draw_grid(screen)
		draw_side_panel(screen, next_tertraminoe, pieces_placed)
